...
sudoku2 :: Form (Int, Int, Int)
sudoku2 = rowsComplete <&&> columnsComplete <&&> squaresComplete
         <&&> rowsNoRepetition <&&> columnsNoRepetition <&&> squaresNoRepetition
...
columnsComplete :: Form (Int,Int,Int) -- swap i and j for rowscomplete
columnsComplete = And [ Or [ P (i, j, n) | i <- [1..9] ]
                      | j <- [1..9], n <- [1..9] ]

-- checks if int n appears at least once within each 3x3 square of the sudoku grid
-- 3*(si - 1) + i (and j) translates the coordinates within the 3x3 into coordinates within the 9x9
squaresComplete :: Form (Int,Int,Int) -- check each 3x3 in each row, then go down the column
squaresComplete = And [ Or [ P (3 * (si - 1) + i, 3 * (sj - 1) + j, n) 
                           | i <- [1..3], j <- [1..3] ]
                      | si <- [1..3], sj <- [1..3], n <- [1..9] ]
...
squaresNoRepetition :: Form (Int,Int,Int) -- does the same thing as above just in a box
squaresNoRepetition = And [ Or [ N (3 * (si-1) + i, 3 * (sj-1) + j, n), 
                                 N (3 * (si-1) + i', 3 * (sj-1) + j', n) ]
                            | si <- [1..3], sj <- [1..3], n <- [1..9], 
                              i <- [1..3], j <- [1..3], 
                              i' <- [1..(i-1)], j' <- [1..3], 
                              (i /= i' || j /= j') ] -- same idea as squarescomplete, two clauses, checks that there's no cases where i, j != i', j' and n1 == n2


solutions :: Form (Int, Int, Int) -> [[Literal (Int, Int, Int)]]
solutions problem = dpll (sudoku <&&> problem)

solutions2 :: Form (Int, Int, Int) -> [[Literal (Int, Int, Int)]]
solutions2 problem = dpll (sudoku2 <&&> problem)
...
1. Every cell must contain an integer from 1..9
2. Every integer in each column, row, and 3x3 square must be unique

Can we improve the efficiency of the solver by removing some of the redundant con-straints?
1. noneFilledTwice - row and column complete guarantee it (each digit only once in each row and column and etc)
2. allFilled - row and column complete guarantee it (same as above)

Compare the efficiency of different sets of constraints using Haskell
sudoku = 4000000
sudoku2 = 2000000

so sudoku2 is faster

-}

main = do
    start <- getCPUTime
    let solution = solutions sudokuProblem 
    end <- getCPUTime
    print (end - start)

    start <- getCPUTime
    let solution = solutions2 sudokuProblem 
    end <- getCPUTime
    print (end - start)
